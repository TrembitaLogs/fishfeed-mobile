...
void main() {
  ...
  group('createAquarium', () {
    test('should create aquarium locally and return it', () async {
      when(
      ...
      result.fold((_) => fail('Should be Right'), (aquarium) {
        ...
        expect(aquarium.userId, 'user-123');
        expect(aquarium.waterType, WaterType.freshwater);
        expect(aquarium.capacity, 50.0);
        expect(aquarium.id, isNotEmpty);
      });
  ...
  group('getAquariums', () {
    test('should return aquariums from local storage', () async {
      when(
        () => mockAuthLocalDataSource.getCurrentUser(),
      ).thenReturn(testUserModel);
    ...
    test('should return empty list when no aquariums exist', () async {
      when(
        () => mockAuthLocalDataSource.getCurrentUser(),
      ).thenReturn(testUserModel);
    ...
    test('should filter out soft-deleted aquariums', () async {
      final deletedModel = AquariumModel(
        id: 'aquarium-deleted',
        userId: 'user-123',
        name: 'Deleted Tank',
        createdAt: DateTime(2024, 1, 15),
  ...
  group('getAquariumById', () {
    ...
    test('should return CacheFailure when not found locally', () async {
      when(
    ...
    test('should return CacheFailure for soft-deleted aquarium', () async {
      final deletedModel = AquariumModel(
        id: 'aquarium-deleted',
        userId: 'user-123',
        name: 'Deleted Tank',
      ...
      when(
        () => mockLocalDataSource.getAquariumById('aquarium-deleted'),
      ).thenReturn(deletedModel);

      final result = await repository.getAquariumById('aquarium-deleted');
  ...
  group('updateAquarium', () {
    test('should return updated aquarium from local storage', () async {
      when(
        () => mockLocalDataSource.getAquariumById('aquarium-123'),
      ).thenReturn(testAquariumModel);
      ...
      result.fold((_) => fail('Should be Right'), (aquarium) {
        ...
        expect(aquarium.capacity, 75.0);
        expect(aquarium.synced, false);
        expect(aquarium.updatedAt, isNotNull);
      });
    ...
    test(
      'should return CacheFailure when aquarium not found locally',
      () async {
        when(
          () => mockLocalDataSource.getAquariumById('aquarium-123'),
        ).thenReturn(null);
  ...
  group('deleteAquarium', () {
    test('should soft delete locally and return unit', () async {
      when(
        () => mockLocalDataSource.softDelete('aquarium-123'),
      ).thenAnswer((_) async {});
