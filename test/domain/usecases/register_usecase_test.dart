...
void main() {
  ...
  group('RegisterUseCase', () {
    ...
    test('should return ValidationFailure when password has no uppercase', () async {
      final result = await useCase(const RegisterParams(
        email: 'new@example.com',
        password: 'password1',
        confirmPassword: 'password1',
    ...
    test('should return ValidationFailure when password has no lowercase', () async {
      final result = await useCase(const RegisterParams(
        email: 'new@example.com',
        password: 'PASSWORD1',
        confirmPassword: 'PASSWORD1',
    ...
    test('should return ValidationFailure when password has no number', () async {
      final result = await useCase(const RegisterParams(
        email: 'new@example.com',
        password: 'Password',
    ...
    test('should return multiple errors for multiple validation failures', () async {
      final result = await useCase(const RegisterParams(
        email: 'invalid',
        password: 'short',
      ...
      result.fold(
        (failure) {
          expect(failure, isA<ValidationFailure>());
          final validationFailure = failure as ValidationFailure;
          expect(validationFailure.errors.keys, containsAll(['email', 'password', 'confirmPassword']));
        },
        (_) => fail('Should be Left'),
      );
    });
