...
void main() {
  ...
  group('GetCalendarDataUseCase', () {
    group('Day Status Calculation', () {
      test('should return allFed when all feedings are completed', () async {
        ...
        final completedEvents = [
          createFeedingEvent(
              id: '1', feedingTime: testDay.add(const Duration(hours: 8))),
          createFeedingEvent(
              id: '2', feedingTime: testDay.add(const Duration(hours: 12))),
          createFeedingEvent(
              id: '3', feedingTime: testDay.add(const Duration(hours: 18))),
          createFeedingEvent(
              id: '4', feedingTime: testDay.add(const Duration(hours: 20))),
        ];

        when(() => mockFeedingDs.getFeedingEventsByDate(any()))
            .thenReturn(completedEvents);

        final result = await useCase(const GetCalendarDataParams(
          year: 2025,
          month: 1,
        ));
    ...
    group('Streak Calculation', () {
      ...
      test('should return 0 streak when no days have all feedings completed',
          () async {
