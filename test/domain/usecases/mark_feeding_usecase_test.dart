...
void main() {
  ...
  group('MarkFeedingUseCase', () {
    group('Validation', () {
      test(
        'should return ValidationFailure when scheduledFeedingId is empty',
        () async {
          ...
          result.fold((failure) {
            ...
            expect(validationFailure.errors['scheduledFeedingId'], isNotEmpty);
      ...
      test(
        'should return ValidationFailure when aquariumId is empty',
        () async {
          final result = await useCase(
            const MarkFeedingParams(
              scheduledFeedingId: 'feeding_1',
    ...
    group('Mark as Fed', () {
      test('should create FeedingEvent when marking as fed', () async {
        ...
        when(
          () => mockFeedingDs.createFeedingEvent(any()),
        ).thenAnswer((_) async {});
        when(() => mockSyncQueueDs.addToQueue(any())).thenAnswer((_) async {});
        when(
        ...
        result.fold((_) => fail('Should be Right'), (markResult) {
      ...
      test(
        'should use aquariumId as fishId when fishId not provided',
        () async {
          final streak = createTestStreak();
          when(
            () => mockFeedingDs.createFeedingEvent(any()),
          ...
          when(
            () => mockSyncQueueDs.addToQueue(any()),
          ).thenAnswer((_) async {});
          when(
            () => mockStreakDs.incrementStreak(any(), any()),
          ).thenAnswer((_) async => streak);
      ...
      test('should add to sync queue when marking as fed', () async {
        ...
        final captured =
            verify(
                  () => mockSyncQueueDs.addToQueue(captureAny()),
                ).captured.single
    ...
    group('Mark as Missed', () {
      ...
      test(
        'should create default streak when none exists and marking as missed',
        () async {
          when(
            () => mockStreakDs.resetStreak(any()),
