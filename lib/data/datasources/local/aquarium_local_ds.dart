...
class AquariumLocalDataSource {
  ...
  Future<void> softDelete(String id) async {
    ...
    await _sentry.captureMessage(
      ...
      tags: {'delete_origin': 'local_soft_delete', 'entity_type': 'aquarium'},
    );
  }
  ...
  Future<void> applyServerUpdate(Map<String, dynamic> serverData) async {
    ...
    if (existing != null) {
      // The server sent this aquarium in server_state → it is alive there. A
      // *synced* local tombstone is therefore stale (the server confirmed our
      // delete earlier but now reports the record alive, or the tombstone was
      // created erroneously). Resurrect it so the user sees their aquarium
      // again. An *unsynced* tombstone is a pending local delete not yet pushed,
      // so it must be left intact to avoid undoing the user's intent.
  ...
  /// Permanently removes soft-deleted aquariums whose deletion the server has
  /// confirmed in this sync (their id appears in [confirmedIds], i.e. the
  /// server's synced/deleted ids).
  ///
  /// Only purging server-confirmed tombstones prevents a synced tombstone for an
  /// aquarium that is still ALIVE on the server from being permanently destroyed
  /// — a delta sync omits unchanged-but-alive rows, so "isDeleted && synced"
  /// alone is not proof the server deleted it.
  Future<void> purgeSyncedDeletions(Set<String> confirmedIds) async {
    ...
    final deleted = _aquariums.values
        .whereType<AquariumModel>()
        .where((a) => a.isDeleted && a.synced && confirmedIds.contains(a.id))
        .toList();
